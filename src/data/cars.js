...
export function addCar(input) {
    ...
    const next = {
        ...
        weightKg: input?.weightKg != null && input.weightKg !== '' ? Number(input.weightKg) : undefined,
        wheelbaseMm: input?.wheelbaseMm != null && input.wheelbaseMm !== '' ? Number(input.wheelbaseMm) : undefined,
        minTurningRadiusM: input?.minTurningRadiusM != null && input.minTurningRadiusM !== '' ? Number(input.minTurningRadiusM) : undefined,
        maxPowerPs: input?.maxPowerPs != null && input.maxPowerPs !== '' ? Number(input.maxPowerPs) : undefined,
        maxPowerKw: input?.maxPowerKw != null && input.maxPowerKw !== '' ? Number(input.maxPowerKw) : undefined,
        maxTorqueNm: input?.maxTorqueNm != null && input.maxTorqueNm !== '' ? Number(input.maxTorqueNm) : undefined,
        displacementL: input?.displacementL != null && input.displacementL !== '' ? Number(input.displacementL) : undefined,
        fuelConsumption: input?.fuelConsumption != null && String(input.fuelConsumption).trim() !== '' ? String(input.fuelConsumption).trim() : undefined,
        driveType: input?.driveType != null && String(input.driveType).trim() !== '' ? String(input.driveType).trim() : undefined,
    };
    writeAll([next, ...all]);
    return next;
}
